...
9. **校园网到 github.com 间歇性失败**：推送失败就重试并核对远端 SHA；`gh` CLI 走 api.github.com 通常可用（开 VPN 最稳）。
10. **老站冷启动曾长达 10-15 秒**（Neon 数据库在新加坡，单次往返 ~300ms × 串行初始化）。已修：`initDb` 用 `site_settings` 的 `schema_ok` 标记跳过重复建表，静态站另有每 5 分钟的 keep-warm ping。⚠️ **以后改老站表结构：把迁移写进 initDb 的 DDL 后，必须删掉 `schema_ok` 标记行，否则新迁移不会执行**。
11. **发码接口 429 时前端仍进入验证码输入页**：老站是先写库发信、后返回，被频控拦住的请求往往验证码已经发出去了——所以静态站在 429 时提示"收到过验证码可直接输入"，别把它改回直接报错。
